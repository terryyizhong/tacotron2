...
def infer(checkpoint_path, griffin_iters, texts):
    ...
    for num, text in enumerate(texts):
        ...
        plot_data((mel_outputs.float().data.cpu().numpy()[0], mel_outputs_postnet.float().data.cpu().numpy()[0],
                    alignments.float().data.cpu().numpy()[0].T), num)
        ...
        audio_path = os.path.join(wav_dir, "{}_synthesis.wav".format(num))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-s', '--steps', type=int,help='griffin lim iters', default=100)
